...
<!-- Modal-->
<div class="modal fade" id="modal-inmueble" data-backdrop="static" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ url('inmueble/guarda') }}" method="POST" id="formulario-tipos">
                    @csrf
                    <div class="row">
...
<div class="card card-custom gutter-b">
    <div class="card-header flex-wrap py-3">
        <div class="card-title">
            <h3 class="card-label">INMUEBLES
            </h3>
        </div>
        <div class="card-toolbar">
            <!--begin::Button-->
            <a href="#" class="btn btn-primary font-weight-bolder" onclick="nuevo()">
                <i class="fa fa-plus-square"></i> NUEVO INMUEBLE
            </a>
            <!--end::Button-->
        </div>
    </div>
    ...
    <div class="card-body">
        ...
        <div class="table-responsive m-t-40">
            <table class="table table-bordered table-hover table-striped" id="tabla-raza">
                ...
                <tbody>
                    @forelse ($inmuebles as $inmu)
                    <tr>
                        <td style="width: 5%">{{ $inmu->id }}</td>
                        <td>{{ $inmu->nombre }}</td>
                        <td>{{ $inmu->descripcion }}</td>
                        <td style="width: 10%">
                            <button type="button" class="btn btn-sm btn-icon btn-warning"
                                onclick="edita('{{ $inmu->id }}', '{{ $inmu->nombre }}', '{{ $inmu->descripcion }}')">
                                <i class="flaticon2-edit"></i>
                            </button>
                            <button type="button" class="btn btn-sm btn-icon btn-danger"
                                onclick="elimina('{{ $inmu->id }}', '{{ $inmu->nombre }}')">
		...
		function elimina(id, nombre)
        {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    window.location.href = "{{ url('inmueble/elimina') }}/"+id;
