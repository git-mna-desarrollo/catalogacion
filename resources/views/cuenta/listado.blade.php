    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">FORMULARIO DE CUENTAS</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i aria-hidden="true" class="ki ki-close"></i>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ url('cuenta/guarda') }}" method="POST" id="formulario-tipos">
                    @csrf
                    <div class="row">
...
<div class="card card-custom gutter-b">
    <div class="card-header flex-wrap py-3">
        <div class="card-title">
            <h3 class="card-label">CUENTAS
            </h3>
        </div>
        <div class="card-toolbar">
            <!--begin::Button-->
            <a href="#" class="btn btn-primary font-weight-bolder" onclick="nuevo()">
                <i class="fa fa-plus-square"></i> NUEVA CUENTA
            </a>
            <!--end::Button-->
        </div>
    </div>
    ...
    <div class="card-body"> 
        ...
        <div class="table-responsive m-t-40">
            <table class="table table-bordered table-hover table-striped" id="tabla-raza">
                ...
                <tbody>
                    @forelse ($cuentas as $cue)
                    <tr>
                        <td style="width: 5%">{{ $cue->id }}</td>
                        <td>{{ $cue->nombre }}</td>
                        <td style="width: 10%">
                            <button type="button" class="btn btn-sm btn-icon btn-warning"
                                onclick="edita('{{ $cue->id }}', '{{ $cue->nombre }}')">
                                <i class="flaticon2-edit"></i>
                            </button>
                            <button type="button" class="btn btn-sm btn-icon btn-danger"
                                onclick="elimina('{{ $cue->id }}', '{{ $cue->nombre }}')">
		...
		function elimina(id, nombre)
        {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    window.location.href = "{{ url('cuenta/elimina') }}/"+id;
